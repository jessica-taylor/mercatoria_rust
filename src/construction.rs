...
pub async fn new_quorum_node_body<HL: HashLookup + HashPut>(
    ...
    // TODO: consider setting total weight limit
    let mut ver_children = Vec::new();
    let mut children_scores = Vec::new();
    for child_hash in possible_children {
        let child = hl.lookup(*child_hash).await?;
        if child.body.path.len() > path.len() && is_prefix(&path, &child.body.path) {
            match verify_endorsed_quorum_node(hl, last_main, &child).await {
                Ok(()) => {
                    let score = quorum_node_body_score(hl, last_main, &child.body).await?;
                    children_scores.push(score);
                    ver_children.push(child.clone());
                }
                Err(_) => {}
            }
        }
